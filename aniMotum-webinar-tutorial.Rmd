...
These can be helpful but are computationally demanding. We calculate the residuals via the `osar` function and can visualise them as a time-series, QQ plot and autocorrelation plot.
...
It is possible to go on to link move persistence to environmental covariates. Take a look at Jonsen et al. ([2019](https://doi.org/10.1002/ecy.2566)) for the method and Grecian et al. ([2022](https://doi.org/10.1098/rsos.211042)) for application to this data set.
...
#p2 <- plot(fit, type = 1, what = "predicted")
#p3 <- plot(fit, type = 2, what = "predicted")
#p4 <- aniMotum::map(fit, what = "predicted")
